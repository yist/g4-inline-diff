...
function! DiffBlocksToQuickFix()
  let fname = bufname("%")
  let block_info = []
  if len(s:diff_blocks[fname]) == 0
    return
  endif

  for block in s:diff_blocks[fname]
    let line = block.id - s:sign_base
    let block_info += [{ 'bufnr': bufnr(''), 'lnum': line, 'text': block.info }]
  endfor
  call setqflist(block_info, 'r')
  copen
endfunction
